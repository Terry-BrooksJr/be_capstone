...
def get_django_forms_templates_path() -> str | None:
    ...
    for site_dir in site.getsitepackages() + site.getusersitepackages().__class__([site.getusersitepackages()]):
...
class Base(Configuration):
    ...
    BYTE_PATROLALLOWED_IPS = _get_env_list(
        "BYTE_PATROLALLOWED_IPS",
        ["127.0.0.1", "69.255.206.86"]
    )
    BYTE_PATROLPROTECTED_PATHS = _get_env_list(
        "BYTE_PATROLPROTECTED_PATHS",
        [r"^/admin/.*$", r"^/checkup/?.*$"]
    )
    BYTE_PATROLCACHE_TIMEOUT = 9000
    if "CACHE_CONNECTION_STRING" not in os.environ:
        raise RuntimeError("CACHE_CONNECTION_STRING environment variable is required for REDIS_URL configuration.")
    REDIS_URL = os.environ["CACHE_CONNECTION_STRING"]
