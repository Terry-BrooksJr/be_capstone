...
from rest_framework.viewsets import reverse_action
...
class BookingSerializerRepresentationTest(TestCase):
    ...
    def test_to_internal_value_parses_date_and_time(self):
        ...
        # Assert
        parsed_datetime = datetime.strptime(serializer.validated_data.get("date", ""), "%m-%d-%Y")
        ...
        self.assertEqual(parsed_datetime.hour, 18)
...
class BookingViewSetTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.booking1 = Booking.objects.create(booking_id=52, name="John Doe", no_of_guests=2, date="2024-05-03T08:00")
        cls.booking2 = Booking.objects.create(name="Jane Doe", no_of_guests=4, date="2024-06-10T19:30")
    ...
    def test_create_booking_valid_and_invalid_cases(self):
        ...
        test_cases = [
            ...
            ({"name": "John Doe", "no_of_guests": 2, "date": "1970-01-01", "time": "18:00"}, status.HTTP_400_BAD_REQUEST),
        ]

        for booking_data, expected_status in test_cases:
            with self.subTest(booking_data=booking_data):
                response = self.client.post(booking_list_url, booking_data, format="json")
    ...
    def test_retrieve_booking_success(self):
        ...
        url = reverse("booking-detail", kwargs={"pk": self.booking1.booking_id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.content, {
            "name": self.booking1.name,
            "no_of_guests": self.booking1.no_of_guests,
            "date": self.booking1.date.strftime("%m-%d-%Y"),
            "time": self.booking1.date.strftime("%I:%M %p")})
