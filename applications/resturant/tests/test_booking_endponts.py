...
class BookingsViewsetTestCase(APITestCase):
    ...
    def test_create_booking_with_valid_data(self):
        response = self.client.post(path=reverse('bookings-create'), data= self.valid_booking_data, format='json', headers={'Authorization': f'Token {self.test_token.key}'})
    ...
    def test_create_booking_with_invalid_data(self):
        response = self.client.post(reverse('bookings-create'), self.invalid_booking_data, format='json')
    ...
    def test_update_booking(self):
        ...
        self.assertEqual(self.booking.date.strftime("%m-%d-%Y %I:%M %p"), datetime.datetime.combine((datetime.datetime.now().date()+datetime.timedelta(days=5)), datetime.time(20, 0)).strftime("%m-%d-%Y %I:%M %p"))
    ...
    def test_partial_update_booking(self):
        ...
        self.assertEqual(self.booking.name, self.booking.name)  # Name should remain unchanged
