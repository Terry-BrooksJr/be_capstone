...
class BookingSerializerRepresentationTest(TestCase):
    ...
    def test_to_internal_value_parses_date_and_time(self):
        ...
        self.assertEqual(
            ...
            (datetime.datetime.now() + datetime.timedelta(days=5)).year,
        )
        self.assertEqual(
            parsed_datetime.month,
            (datetime.datetime.now() + datetime.timedelta(days=5)).month,
        )
        self.assertEqual(
            parsed_datetime.day,
            (datetime.datetime.now() + datetime.timedelta(days=5)).day,
        )
